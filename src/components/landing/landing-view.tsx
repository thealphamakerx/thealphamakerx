...
export function LandingView({ page, data }: { page: LandingPageRecord; data: Loaded }) {
  ...
  const ctaButton = (id?: string) => {
    ...
    return direct
      ...
      : <a id={id} href="#offers" className={className}>{text}</a>;
  };
  ...
  const render: Record<SectionKey, () => React.ReactNode> = {
    ...
    forYou: () => content.forYou.length > 0 && (
      <Section title={content.forYouTitle || "This is for you if…"}>
        <ul className="grid gap-3 sm:grid-cols-2">
          {content.forYou.map((p) => (
            <li key={p} className="lp-card flex items-start gap-3 p-4">{check}<span>{p}</span></li>
          ))}
        </ul>
      </Section>
    ),
    ...
    inside: () => inside.length > 0 && (
      <Section eyebrow={t("insideEyebrow")} title={content.insideTitle || "What's inside"}>
    ...
    bonuses: () => content.bonuses.length > 0 && (
      <Section eyebrow={t("bonusesEyebrow")} title={content.bonusesTitle || "Bonuses you get today"}>
          ...
          {content.bonuses.map((b, i) => (
            <div key={`${b.title}-${i}`} className="lp-card lp-card-glow flex flex-col gap-2 overflow-hidden p-5">
              ...
              <span className="flex items-center gap-2 text-xs font-semibold uppercase tracking-wider text-accent-foreground">
                <Gift className="size-4" aria-hidden="true" /> {t("bonusTag")} #{i + 1}
              </span>
              ...
              {b.value > 0 && (
                <span className="mt-auto text-sm">
                  {t("bonusWorth")} <span className="line-through">{formatPrice(b.value)}</span> — <span className="font-semibold text-success">{t("bonusFree")}</span>
                </span>
              )}
            </div>
          ))}
          ...
          {content.testimonials.map((item, i) => (
            <figure key={`${item.name}-${i}`} className="lp-card break-inside-avoid overflow-hidden">
              {item.videoUrl ? (
                ...
                <img src={withTransform(item.imageUrl, "w-720")} alt={item.name ? `Message from ${item.name}` : "Customer message"} loading="lazy" className="w-full" />
              )}
              {(item.text || item.name) && (
                <figcaption className="flex flex-col gap-2 p-4">
                  {item.text && <blockquote className="text-sm">“{item.text}”</blockquote>}
                  {item.name && <span className="text-xs font-semibold text-muted-foreground">— {item.name}</span>}
                </figcaption>
              )}
            </figure>
          ))}
    ...
    guarantee: () => (content.guaranteeTitle || content.guaranteeText) && (
      <Section>
        <div className="lp-card flex flex-col items-center gap-3 p-6 text-center sm:p-8">
          ...
          {content.guaranteeText && <p className="max-w-xl whitespace-pre-line text-muted-foreground">{content.guaranteeText}</p>}
        </div>
      </Section>
    ),
    ...
    offers: () => (
      <Section id="offers" eyebrow={t("offersEyebrow")} title={singlePack ? t("offersTitleSingle") : t("offersTitle")} className="scroll-mt-4">
        {endsAt && (
          <p className="mb-6 flex flex-wrap items-center justify-center gap-2 text-center text-sm">
            {t("countdown")} <Countdown endsAt={endsAt} className="inline-flex gap-1.5 text-accent-foreground" />
          </p>
        )}
        ...
        <p className="mt-5 text-center text-xs text-muted-foreground">
          {t("offersNote") || `Secure checkout on ${new URL(siteConfig.url).hostname.replace(/^www\./, "")} · UPI, cards & net banking · Instant download`}
        </p>
      </Section>
    ),

    closing: () => (content.closingTitle || content.closingText) && (
  ...
  return (
    <div className="lp flex flex-1 flex-col pb-20">
        ...
        <div className="mx-auto grid w-full max-w-5xl items-center gap-10 px-5 py-12 sm:py-20 md:grid-cols-[1fr_0.8fr]">
          <div className="flex flex-col gap-5 text-center md:text-left">
            ...
            {content.heroBullets.length > 0 && (
              <ul className="flex flex-col gap-2 text-left">
                {content.heroBullets.map((b) => (
                  <li key={b} className="flex items-start gap-2.5 text-sm sm:text-base">{check}<span>{b}</span></li>
                ))}
              </ul>
            )}
            ...
            <div className="flex flex-col items-center gap-3 md:items-start">
              {ctaButton("hero-cta")}
              {t("heroTrust") && <p className="text-xs text-muted-foreground">{t("heroTrust")}</p>}
            </div>
      ...
      {sections.map((s) => <Fragment key={s.key}>{render[s.key]()}</Fragment>)}
