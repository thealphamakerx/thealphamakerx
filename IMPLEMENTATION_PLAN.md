...
- **File storage: Cloudflare R2 (S3-compatible), replacing Neon Object Storage.** A private bucket for this project only, on the existing Cloudflare account, with an API token scoped to that bucket (`R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY`, `R2_BUCKET`; setup in `docs/r2-setup.md`). Each product has a paid file (`digitalFile{Key,Name,Size,UploadedAt}`) and an optional free preview (`previewFile*`), alongside `digitalAccessUrl` (the external-link fallback). Admin uploads go straight from the browser to R2 through a presigned PUT: `POST /api/upload` signs it, `POST /api/upload/complete` HEAD-checks the object and attaches it (deleting any file it replaces), and `DELETE /api/upload` removes a file. Buyers download through `/api/download/[token]/[productId]`. `authorizeDownload` checks the order link, the order, ownership by email, PAID status and that the item is in the order before it signs a 15-minute URL. Previews are served through `/api/preview/[productId]`.
