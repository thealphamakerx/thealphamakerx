...
- **File storage: Neon Object Storage (S3-compatible), replacing the earlier Cloudflare R2 plan.** Private `uploads` bucket declared in `neon.ts`; credentials are the `AWS_*` vars from `neon env pull`. Products carry `digitalFileKey`/`digitalFileName` (object key) alongside `digitalAccessUrl` (external-link fallback, still supported). Admin uploads (PDF/ZIP, 500MB cap) go browser → storage directly via presigned PUT so large files never pass through Next.js: `POST /api/upload` signs the URL, `POST /api/upload/complete` HEAD-checks the object and attaches it (deleting the replaced file). Downloads use `getSignedDownloadUrl` (presigned GET, 5-minute expiry, regenerated per access); the original file name is stored as the object's `Content-Disposition` at upload time.
